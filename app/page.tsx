  ...
  return (
    <div ref={ref} className="px-4 py-10">
      <div className="mx-auto flex max-w-6xl items-center gap-4">
        {/* Left line grows from right to left */}
        <div className="flex-1 overflow-hidden">
          <motion.div
            ...
            style={{
              ...
              height: '1px',
              background: 'repeating-linear-gradient(to right, rgba(201,153,63,0.4) 0, rgba(201,153,63,0.4) 8px, transparent 8px, transparent 16px)',
            }}
          />
        </div>

        {/* Scissors — bounces in then snips forever */}
        <motion.span
          initial={{ opacity: 0, scale: 0, rotate: -60 }}
          animate={visible
            ? { opacity: 1, scale: 1, rotate: [0, -22, 0, -22, 0] }
            : { opacity: 0, scale: 0, rotate: -60 }}
          transition={visible
            ? { opacity: { duration: 0.3, delay: 0.1 }, scale: { duration: 0.4, delay: 0.1, type: 'spring', bounce: 0.5 }, rotate: { delay: 0.7, duration: 1.4, repeat: Infinity, repeatDelay: 1.2, ease: 'easeInOut' } }
            : {}}
          className="shrink-0 select-none text-4xl text-amber-300"
          style={{ filter: 'drop-shadow(0 0 12px rgba(201,153,63,0.7))' }}
        >
          ✂
        </motion.span>

        {/* Right line grows from left to right */}
        <div className="flex-1 overflow-hidden">
          <motion.div
            initial={{ scaleX: 0 }}
            animate={visible ? { scaleX: 1 } : {}}
