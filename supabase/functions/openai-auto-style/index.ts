...
const OPENAI_IMAGE_QUALITY = Deno.env.get("OPENAI_IMAGE_QUALITY") || "medium";
const OPENAI_IMAGE_SIZE = Deno.env.get("OPENAI_IMAGE_SIZE") || "auto";
const OPENAI_IMAGE_OUTPUT_FORMAT =
  Deno.env.get("OPENAI_IMAGE_OUTPUT_FORMAT") || "png";
...
async function fetchImageFile(
  ...
  return new File([buffer], `${fallbackName}.${mimeTypeToExtension(mimeType)}`, {
    type: mimeType,
  });
}
...
async function addCatalogReferenceImage(
  ...
  try {
    references.push({
      label,
      file: await fetchImageFile(url, `reference-${references.length + 1}`),
    });
  } catch (error) {
...
async function insertOpenAICallLog(row: Record<string, unknown>) {
  if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
    console.warn("OPENAI LOG SKIPPED: missing Supabase service env");
    return null;
  }
  ...
  try {
    ...
    if (!response.ok) {
      console.error("OPENAI LOG INSERT FAILED:", {
    ...
    console.error("OPENAI LOG INSERT ERROR:", serializeError(error));
    return null;
  }
}

async function updateOpenAICallLog(id: string | null, patch: Record<string, unknown>) {
  ...
  try {
    ...
    if (!response.ok) {
      console.error("OPENAI LOG UPDATE FAILED:", {
        status: response.status,
        body: await response.text().catch(() => ""),
      });
    }
  } catch (error) {
    console.error("OPENAI LOG UPDATE ERROR:", serializeError(error));
  }
}
...
async function callOpenAIImageEdit(
  ...
  return {
    ...
    mimeType: `image/${OPENAI_IMAGE_OUTPUT_FORMAT}`,
    openaiRequestId: res.headers.get("x-request-id") || null,
  };
}
...
Deno.serve(async (req) => {
  try {
    ...
    const sourceImage = await fetchImageFile(src_file_url);
    let prompt = "";
    ...
    const logContext = (log_context || {}) as OpenAILogContext;
    const logId = await insertOpenAICallLog({
      request_id: logContext.request_id || null,
      user_id: logContext.user_id || null,
      edge_function: "openai-auto-style",
      ...
      prompt,
      gemini_model: OPENAI_IMAGE_MODEL,
      status: "started",
    ...
    try {
      result = await callOpenAIImageEdit(prompt, sourceImage, referenceImages);
      await updateOpenAICallLog(logId, {
        status: "success",
        generated_image_base64: result.imageBase64,
        generated_image_mime_type: result.mimeType,
        error_details: result.openaiRequestId
          ? { openai_request_id: result.openaiRequestId }
          : null,
    ...
    console.error("OPENAI MEN EDGE ERROR:", e);
